...
class ReportTab(ttk.Frame):
    ...
    def _save_report(self, report: str, chart_data: list = None, source_contents: list = None):
        try:
            ...
            # 차트 생성 → 임시 폴더 사용 후 Word 삽입 완료되면 삭제
            import tempfile
            charts_tmp = tempfile.mkdtemp(prefix="confluence_charts_") if chart_data else None
            chart_md_block = ""
            chart_paths = []
            try:
                if chart_data and charts_tmp:
                    ...
                    if chart_paths:
                        ...
                        for cp in chart_paths:
                            # 절대경로 사용 (임시 폴더 기준)
                            chart_md_block += f"[EMBED_IMAGE:{cp.replace(chr(92), '/')}]\n\n"
                ...
                if charts_tmp and os.path.isdir(charts_tmp):
                    import shutil as _chart_shutil
                    _chart_shutil.rmtree(charts_tmp, ignore_errors=True)
