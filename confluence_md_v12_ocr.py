...
class ReportTab(ttk.Frame):
    ...
    def _save_report(self, report: str, chart_data: list = None, source_contents: list = None):
        try:
            ...
            # 차트 생성 후 MD에 삽입
            charts_dir = os.path.join(self.out_var.get(), "charts")
            chart_md_block = ""
            if chart_data:
                chart_paths = generate_charts(chart_data, charts_dir, callback=self.log)
                if chart_paths:
                    chart_md_block = "\n\n## 📊 데이터 시각화\n\n"
                    for cp in chart_paths:
                        rel = os.path.relpath(cp, self.out_var.get()).replace("\\", "/")
