...
def _make_samsung_word(md_sources: list, out_docx: str, title: str = "", callback=None) -> bool:
    ...
    try:
        ...
        for sec in doc.sections:
            ...
            hp.clear()
            hr_txt = hp.add_run(f"{report_title}    |    Samsung Confidential")
            hr_txt.font.name = "맑은 고딕"; hr_txt.font.size = Pt(9); hr_txt.font.color.rgb = DGRAY
            hpPr = hp._p.get_or_add_pPr()
            hBdr = OxmlElement("w:pBdr")
            hbot = OxmlElement("w:bottom")
            hbot.set(qn("w:val"),   "single"); hbot.set(qn("w:sz"), "4")
            hbot.set(qn("w:space"), "1");      hbot.set(qn("w:color"), BLUE_H)
            hBdr.append(hbot); hpPr.append(hBdr)
            # Footer with page number
            ftr = sec.footer
            fp  = ftr.paragraphs[0] if ftr.paragraphs else ftr.add_paragraph()
            fp.clear(); fp.alignment = WD_ALIGN_PARAGRAPH.CENTER
            fp.add_run(f"{datetime.now().strftime('%Y-%m-%d')}    ").font.size = Pt(9)
...
def _parse_md_to_doc(doc, md_text, base_dir,
    ...
    def embed_image(path_str):
        img_abs = (path_str if os.path.isabs(path_str)
                   else os.path.join(base_dir, path_str)) if base_dir else path_str
        if not os.path.exists(img_abs): return
        try:
            p = doc.add_paragraph(); p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            run = p.add_run()
            # Scale to max width 14cm
    ...
    for line in md_text.splitlines():
        ...
        if s.startswith("[EMBED_IMAGE:") and s.endswith("]"):
            embed_image(s[len("[EMBED_IMAGE:"):-1]); continue
        # Headings
