...
class MDConverter:
    ...
    def convert(self, md_text: str, title: str = "", add_cover: bool = True, base_dir: str = ""):
        doc = self.Document()
        WD_COLOR_INDEX = __import__("docx.enum.text", fromlist=["WD_COLOR_INDEX"]).WD_COLOR_INDEX
        CT     = __import__("docx.oxml.ns", fromlist=["qn"]).qn
        OxmlEl = __import__("docx.oxml", fromlist=["OxmlElement"]).OxmlElement
        ...
        def flush_table():
            ...
            data_rows = [r for r in table_rows[1:] if not all(re.match(r"^[-:]+$", c.strip()) for c in r)]
            if not data_rows:
                in_table = False
                table_rows = []
                return
            ...
            t.style = "Table Grid"
            from docx.shared import RGBColor as _RGB, Pt as _Pt
            from docx.oxml.ns import qn as _qn
            from docx.oxml import OxmlElement as _OxE
            # Header row – Samsung Blue bg
            hrow = t.rows[0]
            for ci, cell_text in enumerate(headers[:ncols]):
                ...
                shd.set(_qn("w:fill"), "1428A0")
                tcPr.append(shd)
            for ri, row_data in enumerate(data_rows):
                row = t.rows[ri + 1]
                fill = "F0F2FA" if ri % 2 == 0 else "FFFFFF"
